...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(JSON.stringify({ error: 'Não autorizado. Faça login para usar o chat.' }), {
        status: 401,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (projetosError || precosError) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao buscar dados de contexto no banco de dados.' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
...
${JSON.stringify(precos)}`;
    ...
    if (!geminiApiKey) {
      return new Response(JSON.stringify({ error: 'Chave da API do Gemini não está configurada no servidor.' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao processar a resposta com a Inteligência Artificial.' }), {
        status: 502,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const reply = aiData.candidates?.[0]?.content?.parts?.[0]?.text || "Desculpe, não consegui formular uma resposta clara com os dados disponíveis.";
    ...
    return new Response(JSON.stringify({ error: error.message || 'Erro interno no processamento do chat.' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
