...
function findFieldValue(fields: any, possibleNames: string[]) {
  ...
  for (const name of possibleNames) {
    const match = keys.find(k => k.toLowerCase() === name.toLowerCase() || k.toLowerCase().includes(name.toLowerCase()));
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profile } = await supabaseAdmin.from('usuarios').select('role').eq('id', user.id).single();
    ...
    if (!tenantId || !clientId || !clientSecret) {
      throw new Error('Credenciais do SharePoint ausentes no ambiente. Verifique as variáveis SHAREPOINT_TENANT_ID, SHAREPOINT_CLIENT_ID e SHAREPOINT_CLIENT_SECRET.');
    }
    ...
    // Resolve site ID
    const siteResponse = await fetch('https://graph.microsoft.com/v1.0/sites/lucenera.sharepoint.com:/sites/LUCENERAPROJETOS345', {
    ...
    const projetosList = listsData.value.find((l: any) => l.displayName === 'Organização Projetos' || l.name === 'Organização Projetos');
    const precosList = listsData.value.find((l: any) => l.displayName === 'Tabelas de Preços' || l.name === 'Tabelas de Preços');

    let projetosCount = 0;
    let precosCount = 0;

    if (projetosList) {
      const pItemsRes = await fetch(`https://graph.microsoft.com/v1.0/sites/${siteId}/lists/${projetosList.id}/items?expand=fields`, {
      ...
      if (pItemsRes.ok) {
        ...
        const extractedProjetos = pItems.value.map((item: any) => ({
          ...
          responsavel: findFieldValue(item.fields, ['Responsavel', 'Responsável']) || 'Não Definido',
          arquiteto_responsavel: findFieldValue(item.fields, ['Arquiteto Responsavel', 'Arquiteto Responsável', 'Arquiteto']) || null,
          responsavel_obra: findFieldValue(item.fields, ['Responsavel da Obra', 'Responsável da Obra', 'Responsável Obra']) || null,
          ...
          data_entrada: getDateValue(item.fields, ['Data Entrada', 'Data de Entrada', 'DataEntrada']),
        ...
        const upsertData = uniqueProjetos.map((projeto: any) => {
          const existing = existingProjetos?.find((p: any) => p.nome.toLowerCase() === projeto.nome.toLowerCase());
        ...
        if (upsertData.length > 0) {
          ...
          if (upsertError) throw new Error(`Erro ao salvar projetos no banco de dados: ${upsertError.message}`);
    ...
    if (precosList) {
      const prItemsRes = await fetch(`https://graph.microsoft.com/v1.0/sites/${siteId}/lists/${precosList.id}/items?expand=fields`, {
        headers: { Authorization: `Bearer ${accessToken}`, 'Prefer': 'HonorNonIndexedQueriesWarningMayFailRandomly' }
      });
      if (prItemsRes.ok) {
        const prItems = await prItemsRes.json();
        const extractedPrecos = prItems.value.map((item: any) => ({
          servico: findFieldValue(item.fields, ['Title', 'Servico', 'Serviço']) || 'Serviço Sem Nome',
        ...
        const { data: existingPrecos } = await supabaseAdmin.from('tabela_precos').select('id, servico');

        const upsertData = uniquePrecos.map((preco: any) => {
          const existing = existingPrecos?.find((p: any) => p.servico.toLowerCase() === preco.servico.toLowerCase());
          return existing ? { id: existing.id, ...preco } : preco;
        });

        if (upsertData.length > 0) {
          const { error: upsertError } = await supabaseAdmin.from('tabela_precos').upsert(upsertData);
          if (upsertError) throw new Error(`Erro ao salvar preços no banco de dados: ${upsertError.message}`);
        ...
        throw new Error(`Erro ao buscar itens de Tabela de Preços no SharePoint. Detalhes: ${errTxt}`);
      }
    } else {
